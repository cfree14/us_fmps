...
data <- tibble(biomass=b_vals,
               bbmsy=b_vals/b_msy) %>%
  # Calculate OFL and ABC
  mutate(f_ofl=f_ofl,
         f_abc=f_abc) %>%
  # Gather
  gather(key="value", value="f", 3:ncol(.)) %>%
  # Format value type
  mutate(value=recode_factor(value, "f_ofl"="OFL", "f_abc"="ABC")) %>%
...
# Export data
write.csv(data, file=file.path(outdir, "NPFMC_scallop.csv"), row.names=F)
...
g1 <- ggplot(data, aes(x=biomass, y=f, color=value)) +
  ...
  scale_x_continuous(breaks=c(0, k),
                     labels=c("0", expression("B"["0"]))) +
  # Labels
...
# Export plot
ggsave(g, filename=file.path(plotdir, "figure_hcr_npfmc_scallop.png"),
