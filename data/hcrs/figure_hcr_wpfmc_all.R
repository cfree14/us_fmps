...
# Export data
write.csv(data, file=file.path(outdir, "WPFMC_all.csv"), row.names=F)
...
g2 <- ggplot(data, aes(x=biomass, y=f, color=value)) +
  ...
  scale_y_continuous(lim=c(0, f_ofl*1.1),
                     breaks=c(0, f_act, f_acl, f_abc, f_ofl),
                     labels=c("0", expression("F"["ACT"]), expression("F"["ACL"]),
                              expression("F"["ABC"]), expression("F"["OFL"]))) +
  scale_x_continuous(breaks=c(0, b_msy, k),
                     labels=c("0", expression("B"["MSY"]), expression("B"["0"]))) +
  # Labels
...
# Export plot
ggsave(g, filename=file.path(plotdir, "figure_hcr_wpfmc_all.png"),
